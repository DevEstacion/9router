...
[ -d "$SERVICE_ROOT" ] || { err "SERVICE_ROOT not found after build: $SERVICE_ROOT"; exit 1; }
...
curl -sfI "$BASE_URL/providers/claude.png" >/dev/null || { err "HEAD /providers/claude.png failed"; exit 1; }
echo "  HEAD /providers/claude.png → OK"

echo
log "Done."
echo
log "Next steps:"
log "  - open $BASE_URL/dashboard/cli-tools in a browser to see Claude classifier controls"
log "  - cli menu: 9router → Settings → 'Claude Classifier Compat'"
